 ...
 */
package video.util.segmentation;
...
public class Scissors implements ControllerListener {
	...
	public Scissors(MediaLocator inML, MediaLocator outML, long start[],
			long end[]) {
		this.inML = inML;
		this.outML = outML;
		this.start = start;
	...
	void checkTrackFormats(Processor p) {
		...
		for (int i = 0; i < tc.length; i++) {
			...
			if (tc[i].getFormat().matches(mpgVideo)) {
				preferred = new VideoFormat(VideoFormat.RGB);
