 ...
 *
 */
public class DetectFilter extends SpatialFilter {
	 ...
	 * @param image
	 */
	public DetectFilter(Mask m, BufferedImage image) {
	 ...
	 *
	 */
	public DetectFilter(BufferedImage image) {
...
//		ma.setPoint(2,2,1);
	 ...
	 * @param constant
	 */
	public DetectFilter(BufferedImage image, int constant) {
		super(null, image,"Border Detect");
		Mask ma = new Mask(3);
	...
	private BufferedImage processImage() {
		...
		System.out.println(im.getHeight()+"x"+im.getWidth());
		return out;
	}
...
//				blue = blue + (cor.getBlue() * mp);
		...
		Color orginalColor = new Color(pic.getRGB(c,l));


		red = Math.abs(orginalColor.getRed() - red/6);
		green = Math.abs(orginalColor.getGreen() - green/6);
		blue = Math.abs(orginalColor.getBlue() - blue/6);
...
//		 = new Color(ajusta((red/3)+constant),
